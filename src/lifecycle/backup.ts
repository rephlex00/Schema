...
interface ExportPayload {
	version: 1;
	exportedAt: string;
	autoRefreshedFields: SchemaPlugin["settings"]["autoRefreshedFields"];
...
export async function exportSchemas(plugin: SchemaPlugin): Promise<void> {
	const payload: ExportPayload = {
		...
		autoRefreshedFields: plugin.settings.autoRefreshedFields,
		folderMappings: plugin.settings.folderMappings,
		schemas: plugin.loader.getAll(),
	};
...
export async function importSchemas(plugin: SchemaPlugin): Promise<void> {
	...
	plugin.loader.setAll(payload.schemas);
	if (payload.autoRefreshedFields) {
		plugin.settings.autoRefreshedFields = payload.autoRefreshedFields;
	}
