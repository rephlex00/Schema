...
export class FolderMappingWatcher {
	...
	stop(): void {
		...
		this.refs = [];
		for (const id of this.createTimers) window.clearTimeout(id);
	...
	private async onCreate(file: TFile): Promise<void> {
		...
		const currentType = readTypeKey(
			cache?.frontmatter as Record<string, unknown> | undefined,
			this.plugin.settings.typeKey
		);
		const hasType = currentType !== undefined;
		// Preserve an existing type on create unless the mapping is enforced.
		// Without enforce, this protects template / paste flows that arrive with
		// their own type already set.
		if (hasType && !mapped.enforce) return;
		if (hasType && currentType === mapped.type) return;

		await this.applyType(file, mapped.type);
	}
	...
	private async onRename(file: TFile, oldPath: string): Promise<void> {
		...
		if (currentType === mapped.type) return;

		await this.applyType(file, mapped.type);
	}

	/** Find the folder mapping with the longest matching prefix. */
	private matchFolder(folder: string): FolderMapping | null {
		const mappings = this.plugin.settings.folderMappings;
		let best: FolderMapping | null = null;
		let bestLen = -1;
		for (const [mappedFolder, mapping] of Object.entries(mappings)) {
			const norm = mappedFolder.replace(/\/+$/, "");
			// An empty key would match every vault-root file (folder === "") and,
			// via startsWith, effectively the whole vault. The editor blocks empty
			// keys, but a hand-edited data.json or imported backup can carry one;
			// never let it auto-classify everything.
			if (norm === "") continue;
			if (folder === norm || folder.startsWith(norm + "/")) {
	...
	private async applyType(file: TFile, typeName: string): Promise<void> {
		...
		try {
			// cleanFrontmatter sets the type key itself (and strips/seeds fields),
			// so a separate processFrontMatter write to set it first is redundant.
