...
/**
 * User-defined filters registered from plugin settings. Compiled once at
 * registration; stored as wrappers that catch runtime errors and pass the input
 * through unchanged on failure (so a misbehaving filter doesn't break renders).
 */
const CUSTOM_FILTERS = new Map<string, Filter>();

/** Register a custom filter compiled from a JS body. The body receives `value`
 *  (a string) and should return the transformed value. Returns ok=false with an
 *  error message if the body fails to compile. Re-registering replaces. */
	...
	try {
		...
		CUSTOM_FILTERS.set(name, (s: string) => {
			try {
				...
				console.error(`[schema] custom filter "${name}" failed at runtime:`, e);
				return s;
			}
		});
		return { ok: true };
...
/** Drop all registered custom filters. Used on plugin reload before
 *  re-registering from the current settings. */
 ...
 *  returning the format string to apply (date/time differ only in their default). */
function parseDateExpr(varPart: string): { format: string } | null {
	if (varPart === "date") return { format: "YYYY-MM-DD" };
	if (varPart === "time") return { format: "HH:mm" };
	if (varPart.startsWith("date:")) return { format: varPart.slice(5).trim() };
	if (varPart.startsWith("time:")) return { format: varPart.slice(5).trim() };
	return null;
}
...
export function formatMoment(format: string, date: Date = new Date()): string {
...
/** ISO-8601 week number (1-53) for a date. Shared with the builtin lookup
 *  runtime's Luxon shim so both compute weeks identically. */
