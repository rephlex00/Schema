...
export class TypedWikilinkSuggest extends EditorSuggest<TFile> {
	...
	onTrigger(
		...
		// 4. Look up the file's object-type and resolve. Prefer the live editor
		// buffer over the metadata cache - while typing in the frontmatter block
		// (exactly where this suggester runs) the cache lags the buffer, so a
		// just-set/changed `type:` would otherwise resolve stale or empty.
		const typeKey = this.plugin.settings.typeKey;
		const cache = this.plugin.app.metadataCache.getFileCache(file);
		const typeName =
			readTypeFromEditor(editor, typeKey) ??
			readTypeKey(cache?.frontmatter as Record<string, unknown> | undefined, typeKey);
	...
	selectSuggestion(file: TFile, _evt: MouseEvent | KeyboardEvent): void {
		...
		let end = this.context.end;
		// If this link is already closed (a `]]` follows the cursor before any new
		// `[[`), consume the existing inner text + `]]` so we don't end up with a
		// dangling or doubled `]]` (e.g. completing inside `[[fo|o]]`/`[[Existing]]`).
		const after = editor.getLine(end.line).slice(end.ch);
		const closeIdx = after.indexOf("]]");
		const nextOpen = after.indexOf("[[");
		if (closeIdx >= 0 && (nextOpen < 0 || closeIdx < nextOpen)) {
			end = { line: end.line, ch: end.ch + closeIdx + 2 };
		}
		// Leave the existing `[[` intact; write the basename + closing `]]`.
...
/**
 * Read the object-type value directly from the editor buffer's frontmatter,
 * so it reflects unsaved edits the metadata cache hasn't picked up yet.
 * Returns undefined when there's no frontmatter or no type key set.
 */
function readTypeFromEditor(editor: Editor, typeKey: string): string | undefined {
	if (editor.getLine(0).trim() !== "---") return undefined;
