import { MarkdownView, setIcon, type EventRef } from "obsidian";
...
export class TypeChipPropertyManager {
	...
	start(): void {
		...
		this.metaRef = this.plugin.app.metadataCache.on("changed", () => this.scheduleRefresh());
	...
	private refreshView(view: MarkdownView): void {
		...
		const schema =
			typeof typeName === "string" && typeName.length > 0
				? this.plugin.loader.getResolved(typeName)
				: undefined;

		// Find every type-property cell in this view (edit + reading mode each
		// have their own DOM tree).
		const cells = container.querySelectorAll<HTMLElement>(
			'[data-property-key="type"] .metadata-property-value'
		);
		cells.forEach((cell) => {
			if (!schema || !typeName) {
			...
			this.applyChip(cell, typeName, schema.defaults ?? {});
		});
	}

	private applyChip(cell: HTMLElement, typeName: string, defaults: Record<string, unknown>): void {
		...
		if (currentName === typeName && currentColor === expectedColor && currentIcon === icon) {
			return;
		}
